...
class AutoEncoder(nn.Module):
    ...
    def __init__(self, data_size):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.data_size, int(1 * self.data_size / 4)),
            nn.Tanh(),
            nn.Linear(int(1 * self.data_size / 4), int(1 * self.data_size / 16)),
            nn.Tanh(),
            nn.Linear(int(1 * self.data_size / 16), int(1 * self.data_size / 64)),
        )
        self.decoder = nn.Sequential(
            nn.Linear(int(1 * self.data_size / 64), int(1 * self.data_size / 16)),
            nn.Tanh(),
            nn.Linear(int(1 * self.data_size / 16), int(1 * self.data_size / 4)),
            nn.Tanh(),
            nn.Linear(int(1 * self.data_size / 4), self.data_size)
        )
